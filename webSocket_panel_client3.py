...
async def panel_connect(server_ip,panel_id,api_hook,log_callback,panel_register,panel_deregister,panel_cancel_all_alarm,stp,latest_LED):
     with connect(server_ip) as websocket:

        api_hook(websocket,panel_id)
        
        # panel_register(panel_id)
        await asyncio.gather(receiver(websocket,panel_id,log_callback,panel_register,panel_cancel_all_alarm,stp,latest_LED))
...
def websocket_client_start(server_ip ,panel_id, api_hook,log_callback,panel_register,panel_deregister,panel_cancel_all_alarm,stp,latest_LED):
   asyncio.run(panel_connect(server_ip ,panel_id, api_hook,log_callback,panel_register,panel_deregister,panel_cancel_all_alarm,stp,latest_LED))
